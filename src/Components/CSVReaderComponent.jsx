...
import Papa from "papaparse"; // react library to  parse CSV files
...
const CSVReaderComponent = () => {
  ...
  const toggleDropdown = (id) => {
    setDropdownVisible((prevState) => ({
      ...prevState,
      [id]: !prevState[id], //  if the id is already in dropdownVisible object then it will be toggled otherwise added with true value
    }));
  };
  ...
  const handleTagSelect = (id, selectedTag) => {
    setSelectedTags((prevSelectedTags) => {
      // Check if the selectedTag is already present in the array
      const isTagAlreadySelected =
        prevSelectedTags[id] && prevSelectedTags[id].includes(selectedTag);

      if (isTagAlreadySelected) {
        // If tag is already selected, ignore the selection
        return prevSelectedTags;
      }

      // Add the selectedTag to the array
      return {
        ...prevSelectedTags,
        [id]: prevSelectedTags[id]
          ? [...prevSelectedTags[id], selectedTag]
          : [selectedTag],
      };
    });
  };
          ...
          <div className={styles.mainbox}>
            <div className={styles.dootedlines} onClick={handleDropAreaClick}>
              <div className={styles.divp}>
                ...
                {!selectedFile && (
                  <div className={`${styles.register} ${styles.opp}`}>
                ...
                {selectedFile && (
                  <div className={styles.showfile}>
                    <div className={styles.dropArea}>{fileName}</div>

                    <div className={styles.wrong} onClick={removefile}>
                      remove
                    </div>
                  </div>
                )}
      ...
      <input class="file-input" type="file" name="file" />
            ...
            <div
              className={`${styles.btnn} ${
                parsedData.length > 0 ? styles.clr : ""
              }`}
              onClick={Dataprocess}
            >
              {loading ? (
                ...
                <tbody className={styles.bodytable}>
                  {parsedData.map((row) => (
                    <tr key={row.id}>
                      <td>{row.id}</td>
                      <td>
                        <a href={`https://${row.links}`}>{row.links}</a>
                      </td>
                      <td>{row.prefix}</td>
                      <td>
                          ...
                          {selectedTags[row.id] &&
                            selectedTags[row.id].map((tag) => (
                              <div key={tag} className={styles.selectedTag}>
                                {tag}
                                <div
                                  onClick={() => handleTagRemove(row.id, tag)}
                                >
                                  <svg
                                    width="12"
                                    height="12"
